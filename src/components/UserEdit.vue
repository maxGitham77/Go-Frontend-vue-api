...
(parseInt(String(this.$route.params.userId), 10) !== store.user.id)" class="btn btn-danger" href="javascript:void(0);"
	...
	export default {
		...
		beforeMount() {
			...
			if (parseInt(String(this.$route.params.userId), 10) > 0) {
				...
				fetch(process.env.VUE_APP_API + "/admin/users/get/" + this.$route.params.userId, Security.requestOptions(""))
		...
		methods: {
			submitHandler() {
				...
				fetch(`${process.env.VUE_APP_API}/admin/users/save`, Security.requestOptions(payload))
					.then((response) => response.json())
					.then((data) => {
						if (data.error) {
							notie.alert({
			...
			confirmDelete(id) {
				notie.confirm({
					...
					submitCallback: function () {
						...
						fetch(process.env.VUE_APP_API + "/admin/users/delete", Security.requestOptions(payload))
						.then((response) => response.json())
						.then((data) => {
							if (data.error) {
								/*notie.alert({
